...
def check_bag_sync(bag_path: str):
    print(f"{'='*70}")
    ...
    try:
        ...
        if d_only or c_only:
            ...
            if c_only: print(f"   - 只有 Color 有有的 Counter: {sorted(list(c_only))[:5]} ... (共{len(c_only)}个)")
