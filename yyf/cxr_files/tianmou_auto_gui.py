...
class TianmouAutoApp(tk.Tk):
    def __init__(self):
        ...
        self._timestamp_check_passed = True  # 记录检测是否都通过
        self._shooting_start_time = None  # 拍摄开始时的系统时间
    ...
    def _drain_log_queue(self):
        while True:
            ...
            if color:
                self.log_text.insert(tk.END, line, (color,))
    ...
    def _check_timestamp_at_gpio_low(self, check_point: str) -> bool:
        """
        检测 GPIO 拉低后时间戳是否正确为 0。
        返回 True 表示正常（读到 0），返回 False 表示异常（读到有效数字）。

        check_point: "开头" 或 "结尾"，用于日志标识
        """
        ...
        try:
            ...
            if ts_us > 0:
                # 读到有效数字，异常！红色报错 3 次
                error_msg = f"[TS-CHECK] ===== 【警告】{check_point}检测异常：GPIO 已拉低但仍读到有效时间戳 {ts_us} us，数据可能无效！====="
                for _ in range(3):
                    self._log(error_msg)
                self._timestamp_check_passed = False
                return False

            return True

        except Exception as exc:
            self._log(f"[ERR] 时间戳读取失败: {exc}")
            return True  # 假设正常
    ...
    def on_read_timestamp(self):
        # 从 config.yaml 读取上次保存路径
        config_path = os.path.join(self.repo_root, "yyf", "config.yaml")
        ts_dir = None
        try:
            if os.path.isfile(config_path):
                ...
                if last_dir:
                    ts_dir = last_dir + "/temporal_files"
        except Exception:
            pass

        if not ts_dir or not os.path.isdir(ts_dir):
            self._log(f"[ERR] 时间戳目录不存在: {ts_dir}")
            return
    ...
    def _write_config_save_dir(self, base_dir: str):
        """将用户指定的 base_dir 写入 config.yaml，供 C++ RealSense 读取。"""
        config_path = os.path.join(self.repo_root, "yyf", "config.yaml")
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                cfg = yaml.safe_load(f)
            cfg["last_save_dir"] = base_dir
            cfg["file_dir"] = base_dir + "/realsense"
            with open(config_path, "w", encoding="utf-8") as f:
                yaml.dump(cfg, f, allow_unicode=True)
            self._log(f"[SYS] 已更新 config.yaml: file_dir = {base_dir}/realsense")
        except Exception as exc:
            self._log(f"[ERR] 写入 config.yaml 失败: {exc}")
    ...
    def _start_workflow(self):
        try:
            ...
            tianmou_dir = user_base_dir + "/tianmou"
            ts_dir = user_base_dir + "/temporal_files"
            ...
            self._log(f"[SYS] 保存路径: {user_base_dir}")

            # 写入 config.yaml，供 C++ RealSense 使用
            self._write_config_save_dir(user_base_dir)

            # 重置检测状态
            self._timestamp_check_passed = True
            ...
            # 0.1 GPIO 拉低后检测时间戳（期待为 0）
            self._log("[SYS] 0.1/6 检测时间戳（开头检查）...")
            self._check_timestamp_at_gpio_low("开头")

            # 0.2 预防性清理：删除可能残留的 .rosbag 索引文件
            if os.path.isdir(user_base_dir):
                removed_count = 0
                for f in os.listdir(user_base_dir):
                    if f.endswith(".rosbag"):
                        try:
                            os.remove(os.path.join(user_base_dir, f))
                            removed_count += 1
                        except Exception:
                            pass
                if removed_count > 0:
            ...
            # 记录拍摄开始时的系统时间
            self._shooting_start_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            self._log(f"[SYS] 拍摄开始系统时间: {self._shooting_start_time}")
            ...
            self._log("[SYS] 等待 RealSense 硬件标定与流稳定 (3.5秒)...")
            time.sleep(3.5)

            # 3. 设置天眸保存路径并开始录制
            self._log("[SYS] 3/5 设置天眸保存路径并开始录制...")
            self.ctrl.set_save_addr(tianmou_dir)
            time.sleep(0.5)
    ...
    def _stop_workflow(self):
        ...
        # 5.1 GPIO 拉低后检测时间戳（期待为 0）
        self._log("[SYS] 5.1/5 检测时间戳（结尾检查）...")
        self._check_timestamp_at_gpio_low("结尾")
        ...
        # 输出拍摄结果总结
        if self._timestamp_check_passed:
            self._log("[SYS] ===== 自动化流程已停止 =====", None)
            self._log("[SYS] ================== 本次拍摄成功 ==================", "green")
        else:
            self._log("[SYS] ===== 自动化流程已停止 =====", None)
            self._log("[SYS] ============ 本次拍摄失败（检测异常）============", "red")
    ...
    def _read_and_save_timestamp(self):
        ...
        try:
            ...
            if ts_us > 0:
                ...
                self._log(f"[TS] 对齐拍摄时间戳 (可读): {readable_time}")
                # 显示拍摄开始时的系统时间用于对比
                if self._shooting_start_time:
                    self._log(f"[TS] 拍摄开始系统时间: {self._shooting_start_time}")
