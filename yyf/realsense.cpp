...
// 配置并启动相机
int RealSense::open() {
    printf("RealSense::open() - run_mode = %d\n", run_mode);
    if (run_mode == RunMode::RECORD) {
        ...
        // 设置 Genlock Mode - 遍历所有传感器
        rs2::sensor sensor = devices[0].query_sensors()[0];
        if (sensor.supports(RS2_OPTION_INTER_CAM_SYNC_MODE)) {
            sensor.set_option(RS2_OPTION_INTER_CAM_SYNC_MODE, 4);
            printf("设置 Stereo Module 为 Genlock Mode\n");
        }

        sensor = devices[0].query_sensors()[1];
        if (sensor.supports(RS2_OPTION_INTER_CAM_SYNC_MODE)) {
            sensor.set_option(RS2_OPTION_INTER_CAM_SYNC_MODE, 4);
            printf("设置 RGB Camera 为 Genlock Mode\n");
        }

        // 启用内部原始时间戳 - 对所有传感器设置
        sensor = devices[0].query_sensors()[0];
        if (sensor.supports(RS2_OPTION_GLOBAL_TIME_ENABLED)) {
            sensor.set_option(RS2_OPTION_GLOBAL_TIME_ENABLED, 0);
            printf("Stereo Module 启用原始 timestamp\n");
        }

        sensor = devices[0].query_sensors()[1];
        if (sensor.supports(RS2_OPTION_GLOBAL_TIME_ENABLED)) {
            sensor.set_option(RS2_OPTION_GLOBAL_TIME_ENABLED, 0);
            printf("RGB Camera 启用原始 timestamp\n");
        }
        ...
        printf("RealSense: 即将启动 pipeline...\n");
        try {
            pipe.start(cfg);
        } catch (const rs2::error &e) {
            printf("RealSense pipeline.start 失败: %s\n", e.what());
            return -1;
        }
        printf("RealSense: pipeline 已启动\n");
        return 1;
    } else if (this->run_mode == RunMode::PLAY) {
        ...
        printf("RealSense: 播放 pipeline 已启动\n");
        return 1;
    } else {
        printf("Run Mode 非法, 程序退出\n");
        return -1;
