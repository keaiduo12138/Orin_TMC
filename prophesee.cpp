...
public:
    ...
    void analyze_events(const EventType *begin, const EventType *end) {
        if (begin == end) return;
        ...
        // counting analysis
        int counter = 0;
        for (const EventType *ev = begin; ev != end; ++ev) {
...
int main(int argc, char *argv[]) {
    ...
    cam.ext_trigger().add_callback([&event_analyzer](const Metavision::EventExtTrigger *ev_begin, const Metavision::EventExtTrigger *ev_end) {
        for (const auto *ev = ev_begin; ev != ev_end; ++ev) {
            std::cout << "External Trigger | Timestamp: " << ev->t << " us" 
                      << " | Value: " << ev->p 
                      << " | ID: " << ev->id << std::endl;
        }
        event_analyzer.analyze_events(ev_begin, ev_end);
