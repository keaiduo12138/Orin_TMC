...
class D455GenlockRecorder:
    ...
    def configure_streams(self, serial_number, output_file, width=848, height=480, fps=30):
        """配置流和录制"""
    ...
    def start_recording(self, output_file=None, sync_mode=4,
        ...
        if output_file is None:
            ...
            output_dir = "/projects/cxr_data/2026-3-20/"
            os.makedirs(output_dir, exist_ok=True)
            output_file = os.path.join(output_dir, f"d455_recording_{timestamp}.bag")
        ...
        self.configure_streams(serial, output_file, width=width, height=height, fps=fps)
        ...
        try:
            while True:
                ...
                if show_preview:
                    ...
                    # 水平拼接：左 Color，右 Depth
                    images = np.hstack((color_image, depth_colormap))

                    cv2.putText(images, f"Frame: {frame_count}", (10, 30),
                                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    cv2.putText(images, f"Color: {color_image.shape[1]}x{color_image.shape[0]}  FPS: {fps}", (10, 65),
...
def main():
    ...
    parser.add_argument('-o', '--output', type=str, default=None,
                        help='输出bag文件路径 (默认: /projects/cxr_data/2026-3-20/d455_recording_时间戳.bag)')
    ...
    print(f"分辨率: {args.width}x{args.height}  FPS: {args.fps}")
    print()
